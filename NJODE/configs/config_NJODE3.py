...
BS_dep_obs_skew_noise_dict = {
    ...
    'obs_noise': {
        ...
        'loc': 0.0,
        'scale': 0.3,
        'alpha': 2.0,  # positive => right skew; change sign for left skew
        'noise_at_start': False,
    },
}
...
# ------------------------------------------------------------------------------
# --- Black–Scholes with asymmetric (skewed) noisy observations
BS_dep_obs_skew_noise_models_path = (
    f"{data_path}saved_models_BS_dep_obs_skew_noise/")
param_list_BS_dep_obs_skew_noise = []

for size in [100]:
    for act in ['tanh', 'relu']:
        _nn = ((size, act),)
        param_dict_BS_dep_obs_skew_noise = {
            'epochs': [200],
            'batch_size': [200],
            'save_every': [1],
            'learning_rate': [0.001],
            ...
            'plot_same_yaxis': [True],
            'plot_obs_prob': [True],
            'saved_models_path': [BS_dep_obs_skew_noise_models_path],
        }
        param_list_BS_dep_obs_skew_noise += get_parameter_array(
            param_dict=param_dict_BS_dep_obs_skew_noise)

overview_dict_BS_dep_obs_skew_noise = dict(
    ids_from=1, ids_to=len(param_list_BS_dep_obs_skew_noise),
    path=BS_dep_obs_skew_noise_models_path,
    params_extract_desc=('data_dict', 'network_size', 'readout_nn',
                         'activation_function_1',
                         'hidden_size', 'batch_size', 'which_loss',
                         'input_sig', 'level', 'coord_wise_tau',
...
# ------------------------------------------------------------------------------
# --- Black–Scholes with time-heteroskedastic noisy observations (unweighted)
...
for size in [100]:
    for act in ['tanh', 'relu']:
        ...
        param_dict_BS_dep_obs_hetero_time = {
            ...
            'plot_same_yaxis': [True],
            'plot_obs_prob': [True],
            'weight_by_time_var': [False], # modify for weighted and unweighted
...
for size in [100]:
    for act in ['tanh', 'relu']:
        ...
        param_dict_BS_dep_obs_hetero_state = {
            ...
            'plot_same_yaxis': [True],
            'plot_obs_prob': [True],
            'saved_models_path': [BS_dep_obs_hetero_state_models_path],
        }
...
overview_dict_BS_dep_obs_hetero_state = dict(
    ...
    path=BS_dep_obs_hetero_state_models_path,
    params_extract_desc=('data_dict', 'network_size', 'readout_nn',
                         'activation_function_1',
                         'hidden_size', 'batch_size', 'which_loss',
                         'input_sig', 'level', 'coord_wise_tau',
...
# ------------------------------------------------------------------------------
# --- Black–Scholes with heavy-tailed noisy observations (Student-t, df=2)
BS_dep_obs_heavy_noise_df2_dict = {
    'model_name': "BlackScholes",
    'drift': 2., 'volatility': 0.3,
    'nb_paths': 20000, 'nb_steps': 100,
    'S0': 1., 'maturity': 1., 'dimension': 1,
...
BS_dep_obs_heavy_noise_df2_models_path = (
    f"{data_path}saved_models_BS_dep_obs_heavy_noise_df2/"
)
param_list_BS_dep_obs_heavy_noise_df2 = []

for size in [100]:
    for act in ['tanh', 'relu']:
        _nn = ((size, act),)
        param_dict_BS_dep_obs_heavy_noise_df2 = {
            'epochs': [200],
            'batch_size': [200],
            'save_every': [1],
            'learning_rate': [0.001],
            ...
            'plot_same_yaxis': [True],
            'plot_obs_prob': [True],
            'saved_models_path': [BS_dep_obs_heavy_noise_df2_models_path],
        }
        param_list_BS_dep_obs_heavy_noise_df2 += get_parameter_array(
            param_dict=param_dict_BS_dep_obs_heavy_noise_df2)
